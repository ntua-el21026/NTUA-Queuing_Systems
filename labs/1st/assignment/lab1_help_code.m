...
for index=1:columns(n)
for i=0:(columns(binomial(index,:))-1)
  mean_value_bino(1,index) = mean_value_bino(1,index) + i.*binomial(index,i+1);
endfor
endfor
...
variance = zeros(1,columns(n));
second_moment_bino = zeros(1,columns(n));
for index=1:columns(n)
for i=0:(columns(binomial(index,:))-1)
  second_moment_bino(1,index) = second_moment_bino(1,index) + i.*i.*binomial(index,i+1);
endfor
variance_bino(1,index) = second_moment_bino(1,index) - (mean_value_bino(1,index))^2;
endfor
